...
begin
  ...
  while not querry_pro.Eof do
    ...
    VValue := VValue + '<tr><td>' + querry_pro.FieldByName('id').AsString + '</td>' + sLineBreak+
                       ...
                       '<td>' + '<button type = "button" class= "btn btn-info mr-2" onclick = return pegarpsicao ('+ querry_pro.FieldByName('id').AsString +', "Alterar")">Editar</button>'+  '<button type = "button" class= "btn btn-danger mr-2" onclick = return pegarpsicao ('+ querry_pro.FieldByName('id').AsString +', "Excluir")">Excluir</button>' + '</td>' + sLineBreak + '</tr>';
                       querry_pro.Next;
